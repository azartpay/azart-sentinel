...
def is_valid_azart_address(address, network='mainnet'):
    ...
    azart_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Azart address
        return False

    if (address_version != azart_version):
        return False
...
# shims 'til we can fix the azartd side
def SHIM_serialise_for_azartd(sentinel_hex):
    from models import AZARTD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = AZARTD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in azartd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # azartd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    azartd_hex = serialise(obj)
    return azartd_hex


# shims 'til we can fix the azartd side
def SHIM_deserialise_from_azartd(azartd_hex):
    from models import AZARTD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(azartd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('azart.conf')
