...
class AzartDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some azartd methods
    ...
    @classmethod
    def from_azart_conf(self, azart_dot_conf):
        from azart_config import AzartConfig
        config_text = AzartConfig.slurp_config_file(azart_dot_conf)
        creds = AzartConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = azartlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = azartlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = azartlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_azartd_proto_version_with_sentinel_ping)
