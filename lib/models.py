...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, azartd):
        golist = azartd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_azartd(azartd, item)
    ...
    @classmethod
    def import_gobject_from_azartd(self, azartd, rec):
        import decimal
        import azartlib
        import inflection
        ...
        # shim/azartd conversion
        object_hex = azartlib.SHIM_deserialise_from_azartd(object_hex)
        objects = azartlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from azartd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from azartd, with every run
        ...
        # get/create, then sync payment amounts, etc. from azartd - Azartd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(azartd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from azartd! %s" % e)
            govobj.vote_delete(azartd)
            return (govobj, None)
    ...
    def vote_delete(self, azartd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(azartd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, azartd, signal, outcome):
        import azartlib
        ...
        output = azartd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = azartlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(azartd, signal)

    def sync_network_vote(self, azartd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = azartd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/AzartDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import azartlib
        obj_data = azartlib.SHIM_serialise_for_azartd(self.serialise())
    ...
    def prepare(self, azartd):
        try:
            object_hash = azartd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = AZARTD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not azartlib.is_valid_azart_address(addr, config.network):
    ...
    def hash(self):
        import azartlib
        return azartlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = AZARTD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, azartd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - azartd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, azartd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - azartd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, azartd):
        now = int(time.time())
        return (self.created_at < (now - azartd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, azartd):
        if self.is_expired(azartd):
            return False

        return True

    def is_deletable(self, azartd):
        if self.is_expired(azartd):
            return True
